...
def main():
    ...
    # model
    model = AnimalClassifier(NUM_CLASSES, NUM_PREDICATES).to(DEVICE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        train_bar = tqdm(train_loader, desc = f"[INFO]: Epoch {epoch + 1}/{NUM_EPOCHS} - Training", leave = False)
        ...
        for i, (images, labels) in enumerate(train_bar):
            ...
            train_bar.set_postfix(progress = f"{(i + 1) / len(train_loader) * 100:.6f}%")
        ...
        with torch.no_grad():
            ...
            val_bar = tqdm(val_loader, desc = f"[INFO]: Epoch {epoch + 1}/{NUM_EPOCHS} - Validation", leave = False) # leave = False ensures the log disappears from the terminal after it completes
            ...
            for i, (images, labels) in enumerate(val_bar):
                ...
                val_bar.set_postfix(progress = f"{(i + 1) / len(val_loader) * 100:.6f}%")
    ...
    with torch.no_grad():
        ...
        test_bar = tqdm(test_images, desc = "[INFO]: Testing Images", leave   = True)
        ...
        for i, img_name in enumerate(test_bar):
            ...
            similarities = torch.nn.functional.cosine_similarity(pred_predicates, predicate_matrix.to(DEVICE), dim = 1)
            # computing similarity in order unseen classes may also be compared

            predicted_class = torch.argmax(similarities).item()

            test_predictions.append((img_name, animal_classes[predicted_class]))

            test_bar.set_postfix(progress=f"{(i + 1) / len(test_images) * 100:.6f}%")
